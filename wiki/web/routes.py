...
def edit(url):
	...
	if form.validate_on_submit():
		...
		if os.path.exists(CONTENT_DIR + '/' + url + '.md'):
			form.populate_obj(page)
			page.save()
			pageVersionManager.update_page()
		else:
			form.populate_obj(page)
			page.save()
			pageVersionManager.create_page()
...
@bp.route('/history/<path:url>/', methods=['POST'])
@protect
def history(url):
	pageVersionManager = PageVersionManager(url, current_user.name)
	edits = pageVersionManager.get_edits()
	edit_list = '<ul>'
	for edit in edits:
		version = edit.replace(' ', '_').replace(':', '_')
		edit_list += '<li><a href="' + version + '" id="restorelink">' + edit + ' - ' + edits[edit]['user'] + '</a></li>'
	edit_list += '</ul>'
	return edit_list
...
def delete(url):
	page = current_wiki.get_or_404(url)
	pageVersionManager = PageVersionManager(url, current_user.name)
	pageVersionManager.delete_page()
